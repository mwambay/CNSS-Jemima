...
<style>
    ...
    .alert-list, .suggestion-list { display: grid; }
    .alert-item, .suggestion-item { display: grid; grid-template-columns: 10px minmax(0, 1fr); gap: .7rem; padding: .85rem 1rem; border-bottom: 1px solid #e4e7ec; color: inherit; text-decoration: none; }
    .alert-item:last-child, .suggestion-item:last-child { border-bottom: 0; }
    .alert-item:hover, .suggestion-item:hover { background: #f9fafb; }
    ...
    .queue-strip { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); border-top: 1px solid #e4e7ec; }
    .queue-item { padding: .8rem 1rem; border-right: 1px solid #e4e7ec; }
    .queue-item:last-child { border-right: 0; }
    .queue-item strong { display: block; color: #101828; font-size: 1rem; }
    .queue-item span { color: #667085; font-size: .72rem; }
    ...
    @media (max-width: 640px) {
        .overview-grid, .queue-strip { grid-template-columns: 1fr; }
        .stat, .stat:nth-child(2), .queue-item { border-right: 0; border-bottom: 1px solid #e4e7ec; }
        .stat:last-child, .queue-item:last-child { border-bottom: 0; }
...
<div class="dashboard">
    <section class="dashboard-panel">
        ...
        <div class="queue-strip">
            <div class="queue-item"><strong>{{ $operationalCounts['pending_affiliations'] }}</strong><span>Affiliations en attente</span></div>
            <div class="queue-item"><strong>{{ $operationalCounts['contribution_anomalies'] }}</strong><span>Ecarts de cotisation</span></div>
            <div class="queue-item"><strong>{{ $operationalCounts['open_fraud_alerts'] }}</strong><span>Alertes de controle</span></div>
            <div class="queue-item"><strong>{{ $operationalCounts['missing_salaries'] }}</strong><span>Salaires manquants</span></div>
        </div>
    </section>
